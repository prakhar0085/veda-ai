...
import { IAssignmentSection, ISectionQuestion } from '../models/assessment.model';
...
export const generateAssignmentAI = async (params: IGenerateParams): Promise<IAssignmentSection[]> => {
...
const MATHEMATICS_MOCK_SECTIONS: IAssignmentSection[] = [
  {
...
const SCIENCE_MOCK_SECTIONS: IAssignmentSection[] = [
  {
...
const COMPUTER_SCIENCE_MOCK_SECTIONS: IAssignmentSection[] = [
  {
...
const generateOfflineMock = (params: IGenerateParams): IAssignmentSection[] => {
  const { subject, numberOfQuestions } = params;
  const cleanedSubject = subject.toLowerCase().replace(/[^a-z]/g, '');

  let baseSections: IAssignmentSection[] = [];
  ...
  const clonedSections: IAssignmentSection[] = JSON.parse(JSON.stringify(baseSections));
  ...
  let currentCount = 0;
  const finalSections: IAssignmentSection[] = [];

  for (const section of clonedSections) {
    if (currentCount >= numberOfQuestions) break;

    const finalQuestions: ISectionQuestion[] = [];
